...
/* Bin.Xu@BSP.Kernel.Stability 2020-05-10, simple slabtrce for memleak analysis */
/* Enalbe slabtrace:
...
#if defined(VENDOR_EDIT) && defined(COMPACT_SLUB_TRACK)
...
static unsigned long sort_addrs[OPPO_MEMCFG_SLABTRACE_CNT]; /*sort by caller address */
static struct kmem_cache *pid_cnt_cachep;

int oppo_cnt_pid(int pid){
...
//wen.luo@bsp.kernel.stability ,2020-1-16, sort by pid
			if ((isbypid && !isbystacks) && !strcmp(s->name, sort_cache_name))
				oppo_cnt_pid(track->pid);
			if (isbystacks && !strcmp(s->name, sort_cache_name))
			{
				if (!memcmp(l->addrs, sort_addrs, OPPO_MEMCFG_SLABTRACE_CNT * sizeof(unsigned long)))
				{
				    oppo_cnt_pid(track->pid);
				}
			}
...
static int oppo_memcfg_slabcfg_show(struct seq_file *m, void *p)
{
	mutex_lock(&slab_mutex);
...
{
	return single_open(file, oppo_memcfg_slabcfg_show, NULL);
}
...
static int __init oppo_memcfg_late_init(void)
{
	...
	oppo_memcfg_dir = proc_mkdir("slab_memcfg", NULL);
	if(!oppo_memcfg_dir) {
		pr_info("%s mkdir /proc/slab_memcfg failed\n", __func__);
	} else {
		pEntry = proc_create("slabtrace", 0400, oppo_memcfg_dir, &proc_slabtrace_operations);
		if(!pEntry)
			pr_info("create slabtrace proc entry failed\n");
		pEntry = proc_create("slabcfg", 0600, oppo_memcfg_dir, &proc_slabcfg_operations);
		if(!pEntry)
