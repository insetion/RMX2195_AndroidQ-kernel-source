...
#ifndef VENDOR_EDIT
		/* Zhangkun@BSP.CHG.Basic, 2020/09/29, Add for auto shutdown */
		if(!disable) {
			smb1355_write(chip, MISC_CHGR_UNLOCK_REG, 0xA5);
			smb1355_masked_write(chip, MISC_CHGR_CBOOT_TIME_REG, 0x40, 0x0);
			pr_err("smb1355_set_parallel_charging enable smb1355\n");
		}
#endif
	...
	if (rc < 0) {
		...
		/* Zhangkun@BSP.CHG.Basic, 2020/09/29, Add for auto shutdown */
			if(!disable) {
				msleep(1000);
				smb1355_write(chip, MISC_CHGR_UNLOCK_REG, 0xA5);
				smb1355_masked_write(chip, MISC_CHGR_CBOOT_TIME_REG, 0x40, 0x40);
				pr_err("smb1355_set_parallel_charging enable smb1355  recover 0x16c4\n");
			}
#endif
...
#ifdef VENDOR_EDIT
			/* Zhangkun@BSP.CHG.Basic, 2020/08/17, Add for qc detect and detach and auto shutdown*/
