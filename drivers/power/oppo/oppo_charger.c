...
/*add for emmc log*/
//#include <soc/oppo/oppo_emmclog.h>//debug for bring up 
...
/*mapenglong@BSP.CHG.Basic, 2020/07/01, add for charger */
#ifdef VENDOR_EDIT
...
void batt_temp_zone_arr_init(struct oppo_chg_chip *chip)
{
	...
	batt_temp_zone_arr[BATT_STAT_NORMAL_4].batt_temp_thres_addr = &(chip->limits.qc_temp_little_too_hot_decidegc);
	batt_temp_zone_arr[BATT_STAT_NORMAL_4].default_value = chip->limits.qc_temp_little_too_hot_decidegc;  //42C
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE:
			...
			//#ifdef ODM_HQ_EDIT
			//zhouyi@BSP.CHG.Function, 2020/05/19 add for mmi_charge
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		//#ifdef ODM_WT_EDIT
			...
			/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-07-27, Add runin disable temperture protect */
		...
		//#ifdef ODM_WT_EDIT
		#ifdef CONFIG_DISABLE_TEMP_PROTECT
			/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-07-27, Add for runin test, Avoid shutdown when low soc */
			if (val->intval < 4)
				val->intval = 4;
		#endif /* CONFIG_DISABLE_TEMP_PROTECT */
		//#endif /* ODM_WT_EDIT */
		...
		//#ifdef ODM_WT_EDIT
			/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-07-27, runin disable temperture protect */
		#ifdef CONFIG_DISABLE_TEMP_PROTECT
...
int oppo_chg_parse_charger_dt(struct oppo_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,input_current_led_ma_high_hvdcp",
			&chip->limits.input_current_led_ma_high_hvdcp);
	if (rc) {
		chip->limits.input_current_led_ma_high_hvdcp = chip->limits.input_current_led_ma;
	}
	
	rc = of_property_read_u32(node, "qcom,default_input_current_led_ma_high",
			&chip->limits.default_input_current_led_ma_high);
	if (rc) {
		chip->limits.default_input_current_led_ma_high = chip->limits.input_current_led_ma;
	}
...
//#ifdef ODM_WT_EDIT
	/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-09-03, Modified charging over voltage protect */
	rc = of_property_read_u32(node, "qcom,qc_9v2a_charger_hv_thr",
			&chip->limits.qc_9v2a_charger_hv_thr);
	if (rc < 0) {
		chip->limits.qc_9v2a_charger_hv_thr = 10000;
	}
	rc = of_property_read_u32(node, "qcom,qc_9v2a_charger_recv_thr",
			&chip->limits.qc_9v2a_charger_recv_thr);
	if (rc < 0) {
		chip->limits.qc_9v2a_charger_recv_thr = 9700;
	}
	rc = of_property_read_u32(node, "qcom,qc_9v2a_charger_lv_thr",
			&chip->limits.qc_9v2a_charger_lv_thr);
	if (rc < 0) {
		chip->limits.qc_9v2a_charger_lv_thr = 7600;
	}
//#endif /* ODM_WT_EDIT */
...
#ifdef ODM_HQ_EDIT
	/* wangxianfei@BSP.CHG.Basic, 2020/06/11, vchg limited by tbat */
				...
				switch (chip->tbatt_status_9v2a) {
					...
					case BATT_STAT_NORMAL:
						if(vbatt_higherthan_4180mv)
							...
							chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_normal_fastchg_ma_hight;
						break;
					...
					case BATT_STAT_NORMAL_3:
						chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_little_too_hot_fastchg_ma;
						break;
...
#ifdef ODM_HQ_EDIT
/* mapenglong@ODM.BSP.CHARGE 2020/08/26 Modify for setting charging_current to 1500mA when CDP charging*/
	if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_CDP) && (charging_current > chip->limits.input_current_cdp_ma)){
		charging_current = chip->limits.input_current_cdp_ma;
		charger_xlog_printk(CHG_LOG_CRTI,
			"CDP charging, set charging current = %d\n",
			chip->limits.input_current_cdp_ma);
	}
#endif
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on) &&
		...
		/* mapenglong@ODM.BSP.CHARGE 2020/08/25 Modify for setting charging_current to 500mA when tbatt over 42*/
...
static void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip)
{
	int current_limit = 0;
	#ifdef ODM_HQ_EDIT
	/* zhangchao@ODM.BSP.charge, 2020/06/09,modify for vbus limit */
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)  && !chip->cool_down) {
		...
		#ifdef ODM_HQ_EDIT
		/* zhangchao@ODM.BSP.charger, 2020/07/15,disable vbus limit 5V when ledon */
		//vbus_limit = VBUS_LIMIT_5V;
		#endif
	...
	charger_xlog_printk(CHG_LOG_CRTI,
		...
		vbus_limit);
	#ifdef ODM_HQ_EDIT
	/* zhangchao@ODM.BSP.charge, 2020/06/09,priority is given to set_vbus due to aicl_point */
	if(chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC){
		...
		if(chip->pre_charger_type != CHARGER_SUBTYPE_QC && vbus_limit == VBUS_LIMIT_9V){
			chip->pre_charger_type = CHARGER_SUBTYPE_QC;
		}
		#endif
...
void oppo_chg_turn_on_ffc1(struct oppo_chg_chip *chip)
{
	...
	if (chip->temperature >= chip->limits.ffc2_temp_warm_decidegc) {
		...
		chip->limits.temp_little_cool_fastchg_current_ma
			= chip->limits.ff1_warm_fastchg_ma;
		#ifdef ODM_HQ_EDIT
		/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/07/13 Add for 12C to 16C FFC parameters */
		chip->limits.temp_little_cool_fastchg_current_ma_high
			= chip->limits.ff1_warm_fastchg_ma;
		chip->limits.temp_little_cool_fastchg_current_ma_low
			= chip->limits.ff1_warm_fastchg_ma;
...
#ifdef ODM_HQ_EDIT
/* wangxianfei@BSP.CHG.Basic, 2020/06/11, disable shake in future lower fcc by swjeita */
static bool swjeita_noshake_lower_fcc(struct oppo_chg_chip *chip)
{
	int batt_temp = chip->temperature;
	...
	switch (tbatt_status) {
		...
		case BATTERY_STATUS__LITTLE_COLD_TEMP:
			...
			charger_xlog_printk(CHG_LOG_CRTI,
					"vbatt_higherthan_4180mv [%d], future_fcc[%d]\n",
					vbatt_higherthan_4180mv, future_fcc);
			break;
		case BATTERY_STATUS__COOL_TEMP:
			if (vbatt_higherthan_4180mv) {
		...
		case BATTERY_STATUS__NORMAL:
			...
			chg_debug("normal future_fcc=%d\n",future_fcc);
			break;
		case BATTERY_STATUS__WARM_TEMP:
			future_fcc = chip->limits.temp_warm_fastchg_current_ma;
			chg_debug("warm future_fcc=%d\n",future_fcc);
			break;
		default:
			break;
	}
	...
	pr_err("chip->qc_lower_fcc_noshake_support = %d, chip->limits.qc_temp_little_hot_5v_support = %d\n",
			chip->qc_lower_fcc_noshake_support,chip->limits.qc_temp_little_hot_5v_support);
...
//#ifdef ODM_WT_EDIT
	/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-09-08, Modified charging over voltage protect */
	if(chip->qc_9v2a_support && (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC)) {
		if (chg_volt > chip->limits.qc_9v2a_charger_hv_thr) { /* > 10000 */
			vchg_counts++;
		...
		} else  if ((chg_volt <= chip->limits.qc_9v2a_charger_recv_thr) && (chg_volt > chip->limits.qc_9v2a_charger_lv_thr)) { /* 7600 ~ 9700 */
			vchg_counts = 0;
			ret = true;
			vchg_status = CHARGER_STATUS__GOOD;
	#if 0
		/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-10-09, Delete for QC3.0 */
		} else  if ((chg_volt <= chip->limits.qc_9v2a_charger_lv_thr) && (chg_volt > chip->limits.charger_hv_thr)) { /* 5800 ~ 7600 */
			vchg_counts++;
			if (vchg_counts >= VCHG_CNT) {
				vchg_counts = 0;
				ret = false;
	...
	#endif
		} else  if (chg_volt <= chip->limits.charger_recv_thr) { /* < 5500 */
			vchg_counts = 0;
			ret = true;
			vchg_status = CHARGER_STATUS__GOOD;
		}
	} else if (chg_volt > chip->limits.charger_hv_thr) {
//#endif /* ODM_WT_EDIT */
		vchg_counts++;
		if (vchg_counts >= VCHG_CNT) {
			vchg_counts = 0;
...
void oppo_chg_variables_reset(struct oppo_chg_chip *chip, bool in)
{
	...
	chip->limits.qc_input_current_charger_ma
		= chip->limits.default_qc_input_current_charger_ma;
	chip->limits.input_current_led_ma_high= chip->limits.default_input_current_led_ma_high;
	reset_mcu_delay = 0;
#ifndef CONFIG_OPPO_CHARGER_MTK
...
#ifdef VENDOR_EDIT
	/* Zhangkun@BSP.CHG.Basic, 2020/08/17, Add for qc detect and detach */
	chip->pre_charger_type= POWER_SUPPLY_TYPE_UNKNOWN;
#endif
	chip->charger_volt = 0;
	chip->vchg_status = CHARGER_STATUS__GOOD;
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_ibus_slave){
		chip->ibus_slave = chip->chg_ops->get_ibus_slave();
	} else {
		chip->ibus_slave = -1;
	}
	if (ui_soc_cp_flag == 0) {
...
/* mapenglong@ODM.BSP.charge, 2020/07/09, Modify for report once every 5 seconds when Vbat lower than 3.3V */
		else if(!vbatt_lowerthan_3300mv)
		{
			return;
		}
	}
...
static int oppo_chg_check_sw_full(struct oppo_chg_chip *chip)
{
	...
	#ifndef ODM_HQ_EDIT
	/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/06/28 Battery is discharging after battery full as usb suspended */
	int flag;
	...
	#ifndef ODM_HQ_EDIT
	/* Yuzhe.Peng@ODM.BSP.CHARGE 2020/06/28 Battery is discharging after battery full as usb suspended */
	if(chip->sw_full && chip->charger_exist && chip->charger_type == 4){
		charger_xlog_printk(CHG_LOG_CRTI, "usb chg suspend after batt full !!\n");
		ret.intval = 1;
		flag = power_supply_set_property(chip->batt_psy,
				POWER_SUPPLY_PROP_INPUT_SUSPEND, &ret);
		if(flag < 0)
			pr_err("Unable to suspend!");
	}else{
		charger_xlog_printk(CHG_LOG_CRTI, "usb chg reset suspend !!\n");
		ret.intval = 0;
		flag = power_supply_set_property(chip->batt_psy,
				POWER_SUPPLY_PROP_INPUT_SUSPEND, &ret);
		if(flag < 0)
...
static bool oppo_chg_check_vbatt_is_full_by_sw(struct oppo_chg_chip *chip)
{
	...
	/* mapenglong@ODM.BSP.charge, 2020/07/16, Modify for remove Vbat_over warning*/
	if (!chip->check_batt_full_by_sw) {
		return false;
	}
...
/*Xianfei.Wang@ODM_HQ.BSP.Charge, 2020/05/06, Add for charge type of hvdcp*/
		charger_xlog_printk(CHG_LOG_CRTI,
			...
			" STATUS[ 0x%x / %d / %d / %d / %d / 0x%x ],"
			" OTHER[ %d / %d / %d / %d / %d/ %d ],"
			" DEVICETREE[ %d ], ODM[ C%d / S%d ]\n",
			chip->charger_exist, chip->charger_type, chip->charger_volt,
			chip->prop_status, chip->boot_mode,
			chip->batt_exist, chip->batt_full, chip->chging_on, chip->in_rechging,
			chip->charging_state, chip->total_time,
			chip->temperature, chip->batt_volt, chip->batt_volt_min, chip->icharging,
			chip->ibus, chip->ibus_slave, chip->soc, chip->ui_soc, chip->soc_load, chip->batt_rm,
			chip->vbatt_over, chip->chging_over_time, chip->vchg_status,
			chip->tbatt_status, chip->stop_voter, chip->notify_code,
			chip->otg_switch, chip->mmi_chg, chip->boot_reason, chip->boot_mode,
			chip->chargerid_volt, chip->chargerid_volt_got,
...
#ifndef VENDOR_EDIT
/* wangdengwen@oppo.com, 2020/3/2, add for batt temprature anti shake handle */
...
#ifndef VENDOR_EDIT
/*mapenglong@BSP.CHG.Basic, 2020/07/01, add for charger */
static bool oppo_chg_get_qc_tbatt_status(struct oppo_chg_chip *chip)
{
	int batt_temp             = chip->temperature;
	...
	int i=0;
	OPPO_CHG_QC_TEMP_STATUS        eTempStatus    = QC_TEMP_STATUS__INVALID;
	static OPPO_CHG_QC_TEMP_STATUS ePreTempStatus = QC_TEMP_STATUS__INVALID;

	if (batt_temp > chip->limits.warm_bat_decidegc) { /*45C*/
		eTempStatus = QC_TEMP_STATUS__INVALID;
	} else if (batt_temp >= chip->limits.qc_temp_little_too_hot_decidegc) { /*42C*/
		eTempStatus = QC_TEMP_STATUS__TOO_HOT;
	} else if (batt_temp >= chip->limits.qc_temp_little_hot_decidegc) { /*40C*/
		eTempStatus = QC_TEMP_STATUS__HOT;
	} else if (batt_temp >= chip->limits.qc_temp_warm_decidegc) { /*37C*/
		eTempStatus = QC_TEMP_STATUS__LITTLE_HOT;
	} else if (batt_temp >= chip->limits.qc_temp_normal_decidegc) {	/*34C*/
		eTempStatus = QC_TEMP_STATUS__WARM;
	} else if (batt_temp >= chip->limits.qc_temp_cool_decidegc + 1) { /*22 + 1C*/
		eTempStatus = QC_TEMP_STATUS__NORMAL;
	} else if (batt_temp >= chip->limits.normal_bat_decidegc) {	/*16C*/
		eTempStatus = QC_TEMP_STATUS__COOL;
	} else {
		eTempStatus = QC_TEMP_STATUS__INVALID;
	}
#ifdef VENDOR_EDIT
	/* wangdengwen@oppo.com, 2020/3/2, add for batt temprature anti shake handle */
	chg_debug(" eTempStatus=%d ePreTempStatus=%d iPreTemp=%d batt_temp=%d\n",eTempStatus, ePreTempStatus, iPreTemp, batt_temp);
	if(isFistCheck)
	{
		iPreTemp    = batt_temp;
		isFistCheck = false;
	...
	if(QC_TEMP_STATUS__INVALID == eTempStatus)
	{
		return isChangeStatus;
	}

	if(ePreTempStatus != eTempStatus)
	{
		chg_debug("temp_state eTempStatus=%d ePreTempStatus=%d\n",eTempStatus, ePreTempStatus);
		isStatusHold   = true;
		if((iPreTemp < batt_temp) && (batt_temp < g_arryQcTmpStep[eTempStatus]) && (batt_temp >= g_arryQcTmpStep[eTempStatus-1]))
		{
			iTempDownCount = 0; //¼ì²âµ½ÎÂ¶ÈÉÏÉýÇÒÎÂ¶È×´Ì¬¸Ä±ä·µ»Øtrue
			isChangeStatus = true;
			chg_debug("iPreTemp=%d, batt_temp=%d, g_arryQcTmpStep[eTempStatus]=%d, g_arryQcTmpStep[eTempStatus-1]=%d\n",
				iPreTemp,batt_temp,g_arryQcTmpStep[eTempStatus], g_arryQcTmpStep[eTempStatus-1]);
		}
		else if(iPreTemp > batt_temp )
		{
			iTempDownCount++;   //Á¬ÐøDOWNCOUNT´Î¼ì²âµ½ÎÂ¶ÈÏÂ½µÇÒÎÂ¶È×´Ì¬¸Ä±ä·µ»Øtrue
			if(iTempDownCount > DOWNCOUNT)
			{
				iTempDownCount = 0;
				isChangeStatus = ((batt_temp < (g_arryQcTmpStep[eTempStatus] - QC_TEMP_JEITA) || (chip->qc_lower_fcc_noshake_support &&
					batt_temp <= g_arryQcTmpStep[QC_TEMP_STATUS__COOL])) && batt_temp >= (g_arryQcTmpStep[eTempStatus-1]))? true: false;
				chg_debug("***iPreTemp:%d,batt_temp%d,[g_arryQcTmpStep[eTempStatus] - QC_TEMP_JEITA]%d,[g_arryQcTmpStep[eTempStatus - 1]]=%d,ischange=%d***\n",
					iPreTemp,batt_temp,g_arryQcTmpStep[eTempStatus] - QC_TEMP_JEITA,g_arryQcTmpStep[eTempStatus - 1],isChangeStatus);
				//***iPreTemp:220,batt_temp214,[g_arryQcTmpStep[eTempStatus] - QC_TEMP_JEITA]210,[g_arryQcTmpStep[eTempStatus - 1]]=-1000,ischange=0***<5>[ 3097.096812]  (0)[22791:kworker/0:1][OPPO_CHG][oppo_chg_get_qc_tbatt_st
			}
			else
			{
				isStatusHold = false;
	...
	ePreTempStatus = isChangeStatus? eTempStatus : ePreTempStatus;
#endif	
	return isChangeStatus;
}
#endif
...
#ifndef VENDOR_EDIT
/*mapenglong@BSP.CHG.Basic, 2020/07/01, add for charger */
static void oppo_chg_qc_temp_to_current(struct oppo_chg_chip *chip)
{
#ifdef VENDOR_EDIT
	/* wangdengwen@oppo.com, 2020/3/2, changed for batt temprature check and anti shake handle */
/* Liulitian@ODM.BSP.charge, 2020/3/31, for charging current incorrect when vbat is over 4180mV at temp 22~34. */
	if(!oppo_chg_get_qc_tbatt_status(chip) && !chip->vbat_4180mv_status_changed)
	{
		return;
	}
#endif
...
/* Liulitian@ODM.BSP.charge, 2020/3/31, for charging current incorrect when vbat is over 4180mV at temp 22~34. */
	...
	if (chip->temperature >= chip->limits.normal_bat_decidegc//16C
		...
		&& chip->temperature < chip->limits.qc_temp_normal_decidegc) {//34C
		if(vbatt_higherthan_4180mv){
			chip->limits.temp_normal_fastchg_current_ma
				= chip->limits.qc_temp_normal_fastchg_ma_low;
		} else {
	...
	} else if (chip->temperature >= chip->limits.qc_temp_normal_decidegc//34
		&& chip->temperature < chip->limits.qc_temp_warm_decidegc) {	//37
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_warm_fastchg_ma;//3300mA
	} else if (chip->temperature >= chip->limits.qc_temp_warm_decidegc	//37
		&& chip->temperature < chip->limits.qc_temp_little_hot_decidegc) {//40
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_little_hot_fastchg_ma;//2900mA
	} else if (chip->temperature >= chip->limits.qc_temp_little_hot_decidegc//40
		&& chip->temperature < chip->limits.qc_temp_little_too_hot_decidegc) {//42
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_little_too_hot_fastchg_ma;//2700mA
	} else if (chip->temperature >= chip->limits.qc_temp_little_too_hot_decidegc//42
		&& chip->temperature < chip->limits.warm_bat_decidegc) {
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_hot_fastchg_ma;
	}

	chg_err(" qc_temp_little_hot_decidegc = %d, temp_warm_fastchg_current_ma= %d\n",
		chip->limits.qc_temp_little_hot_decidegc,chip->limits.temp_warm_fastchg_current_ma);
	...
	if (qc_chging == false
		...
		if (ret >= 0) {
			...
			chip->limits.input_current_charger_ma
				= chip->limits.qc_input_current_charger_ma;
			#ifdef ODM_HQ_EDIT
			/* zhangchao@BSP.CHG.Basic, 2020/07/16, add for QC charger */
			chip->limits.input_current_led_ma_high = chip->limits.input_current_led_ma_high_hvdcp;
			#endif
			oppo_chg_set_input_current_limit(chip);
			oppo_chg_set_charging_current(chip);
		}
...
/* wangxianfei@BSP.CHG.Basic, 2020/06/15, swjeita vchg limited by this api */
		chg_err("qc_9v2a_support\n");
		chip->chg_ops->chg_qc_temp_to_vchg();
#endif

#ifndef ODM_HQ_EDIT
/*mapenglong@BSP.CHG.Basic, 2020/07/01, add for charger */
		oppo_chg_qc_temp_to_current(chip);
#endif
		oppo_chg_set_charging_current(chip);
	}
}
...
//#ifdef ODM_WT_EDIT
/* Bin2.Zhang@ODM_WT.BSP.Charger.Basic, 2020-08-13, Add for monitor skin/cpu temperature */
#include <linux/iio/consumer.h>
static void odm_get_thermal_temp(struct oppo_chg_chip *chip)
{
...
void oppo_smart_charge_by_cool_down(struct oppo_chg_chip *chip, int val)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "val->intval = [%04x], set cool_down = [%d], cool_down_done=%d\n", 
		val, chip->cool_down, chip->cool_down_done);
}
